...
local btn=Obj.ScreenGUI_HiddenChest.Button_main
...
testForHiddenChest = function()
	if not(Raycast:getResult().HitObject==nil) then
		if Raycast:getResult().HitObject.Name=="HiddenChest_Cube" then 
			--print(allowContact)
...
Raycast:addFunc(testForHiddenChest)
...
--[[
    ...
    if Input.GetPressKeyData(Enum.KeyCode.Mouse0) == 1 then --Input.GetPressKeyData（）的作用是获取某个按键的状态  Enum.KeyCode.Mouse0--鼠标左键
		...
		for i,v in pairs(HitResults.HitObjectAll) do                 --循环击中点
            --这里是满足条件，打开GUI
			if v.Block and HitResults.HitObject.Name=="HiddenChest_Cube" then    --v.Block是？？射线是否触碰的bool值？
                btn:SetActive(true)
				btn.Figure:SetActive(true)
				btn.Button_leave:SetActive(true)
				print("button_interview出现了！")
