...
local btn=obj.ScreenGUI_interview.Figure				--找到存放GUI的Figure（文件夹作用）
btn:SetActive(false)                            --关闭GUI图片
btn.Button_leave:SetActive(true)
allowContact = false

local Camera = localPlayer.Local.ConstraintFree.Cube.GameCamera             --获取到摄像机
--[[
Input.OnKeyDown:Connect(function()              --Input.OnKeyDown  函数在有按键被按下时触发
    if Input.GetPressKeyData(Enum.KeyCode.Mouse0) == 1 then --Input.GetPressKeyData（）的作用是获取某个按键的状态  Enum.KeyCode.Mouse0--鼠标左键
		...
		for i,v in pairs(HitResults.HitObjectAll) do                 --循环击中点
            --这里是满足条件，打开采访的GUI
			if v.Block and HitResults.HitObject.Name=="G_Book_Open_03_Instance" then    --v.Block是？？射线是否触碰的bool值？
                btn:SetActive(true)
				btn.Button_leave:SetActive(true)
				print("button_interview出现了！")
				--localPlayer.Local.ConstrainFree.Cube.GameCamera.EnableMouseDrag = false
	...
	if not(Raycast:getResult().HitObject==nil) then
		if Raycast:getResult().HitObject.Name=="Item" then 
			...
			if not(Raycast:getResult().HitObject.ID ==nil) then
				if Raycast:getResult().HitObject.ID.Value == "A6"  then
...
Input.OnKeyDown:Connect(function()              --Input.OnKeyDown  函数在有按键被按下时触发
	...
	--Input.GetPressKeyData（）的作用是获取某个按键的状态  Enum.KeyCode.Mouse0--鼠标左键
